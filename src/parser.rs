...
pub fn extract_code_languages(page: &Page) -> Vec<&str> {
    page.code_blocks.iter().filter_map(|cb| cb.language.as_deref()).collect()
}
...
mod tests {
    ...
    #[test] fn test_parse_title() { let d = Html::parse_document(sample()); assert_eq!(parse_title(&d), "Rust"); }

    #[test] fn test_parse_code_blocks() {
    ...
    #[test] fn test_extract_languages() {
        let p = Page { url: "".into(), title: "".into(), content: "".into(),
            code_blocks: vec![CodeBlock { language: Some("rust".into()), code: "a".into() }],
            menu: vec![] };
        assert_eq!(extract_code_languages(&p), vec!["rust"]);
    }

    #[test] fn test_count_by_language() {
        let p = Page { url: "".into(), title: "".into(), content: "".into(),
            code_blocks: vec![
                CodeBlock { language: Some("rust".into()), code: "a".into() },
                CodeBlock { language: Some("rust".into()), code: "b".into() },
                CodeBlock { language: Some("python".into()), code: "c".into() },
            ],
            menu: vec![] };
    ...
    #[test] fn test_empty_parse_fails() { assert!(parse_page("https://x.com", "<html></html>").is_err()); }

    #[test] fn test_page_has_code() {
        let w = Page { url: "".into(), title: "".into(), content: "".into(),
            code_blocks: vec![CodeBlock { language: None, code: "x".into() }], menu: vec![] };
        assert!(w.has_code());
        let wo = Page { url: "".into(), title: "".into(), content: "".into(),
            code_blocks: vec![], menu: vec![] };
        assert!(!wo.has_code());
    }

    #[test] fn test_page_word_count() {
        let p = Page { url: "".into(), title: "".into(), content: " a b  c ".into(),
            code_blocks: vec![], menu: vec![] };
        assert_eq!(p.word_count(), 3);
    }

    #[test] fn test_menu_classification() {
        let s = MenuItem { title: "S".into(), url: "/".into(),
            children: vec![MenuItem { title: "C".into(), url: "/c".into(), children: vec![], active: false }],
            active: false };
        assert!(s.is_section());
        assert!(!s.is_page());
        let p = MenuItem { title: "P".into(), url: "/p".into(), children: vec![], active: false };
        assert!(p.is_page());
    }

    #[test] fn test_page_summary() {
        let p = Page { url: "".into(), title: "T".into(), content: "one two".into(),
            code_blocks: vec![CodeBlock { language: None, code: "x".into() }], menu: vec![] };
        let s = p.summary();
        assert_eq!(s.title, "T");
    ...
    #[test] fn test_resolve_base() { assert_eq!(resolve_base_url("https://metanit.com/rust/"), "https://metanit.com"); }
}
